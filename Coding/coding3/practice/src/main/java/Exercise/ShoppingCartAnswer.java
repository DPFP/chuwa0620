...
public class ShoppingCartAnswer {
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                ...
                .map(items -> items.get(0))
                .map(Item::getName)
                .orElseThrow(() -> new EmptyCartException("Shopping cart is empty or doesn't exist"));
    }
     ...
     */
    public static void checkItemInCart(Customer customer) {
        Optional<List<Item>> itemsOptionnal = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);
        if (itemsOptionnal.isPresent() && !itemsOptionnal.get().isEmpty()) {
