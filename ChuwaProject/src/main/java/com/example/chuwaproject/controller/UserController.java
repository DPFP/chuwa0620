...
public class UserController {
    ...
    public ResponseEntity<UserDTO> login(@RequestBody Map<String,String> credentials){
        ...
        if(userOptional.isPresent() && userOptional.get().getPassword().equals(credentials.get("password"))){
            ...
            // Set cookies (example: setting a session ID cookie)
            String sessionId = Math.random()+"";
            Cookie cookie = new Cookie("JSESSIONID", sessionId);
            cookie.setMaxAge(3600); // Set cookie expiration time in seconds (1 hour in this case)
            cookie.setPath("/"); // Set cookie path to be valid for the entire application context
            // You can also set other properties of the cookie if needed (e.g., domain, secure, etc.)

            // Add the cookie to the response headers
            responseHeaders.add(HttpHeaders.SET_COOKIE, cookie.toString());
            logger.info("Login successful for email: {}", credentials.get("email"));
            return ResponseEntity.ok().headers(responseHeaders).body(AllUtils.mapToUserDTO(userOptional.get()));
        }
